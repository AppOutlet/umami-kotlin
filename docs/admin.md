...
!!! info "New to Umami Kotlin?"
    Before using the Admin API, ensure you have initialized the `UmamiApi` client. Refer to our getting started guides for [Kotlin Multiplatform](../getstarted/kmp.md) and [Android](../getstarted/android.md) for setup instructions.
...
You can access the `Admin` API functionalities through an extension function on your `UmamiApi` client instance:

```kotlin
// Assuming 'api' is an initialized UmamiApi client
val adminApi = api.admin()
```
